...
export default function PosPage() {
  ...
  const [payMethod, setPayMethod] = React.useState<"CASH" | "CARD" | "VOUCHER">("CARD");
  ...
  const [payAmountInput, setPayAmountInput] = React.useState("");
  React.useEffect(() => {
    setPayAmountInput((cartTotal / 100).toFixed(2).replace(".", ","));
  }, [cartTotal]);
  ...
  async function finalizeSale() {
    ...
    const amount = parsePLNToGrosze(payAmountInput);
    if (amount === null || amount <= 0) return toast.error("Nieprawidłowa kwota płatności");
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-2">
                ...
                <Input value={payAmountInput} onChange={(e) => setPayAmountInput(e.target.value)} />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl bg-zinc-50 p-3 text-sm font-semibold dark:bg-white/5">
              <span>Suma</span>
              <span>{formatPLNFromGrosze(cartTotal)}</span>
            </div>

            <Button
              className="w-full"
              disabled={submitting || cart.length === 0 || !warehouseId}
        ...
        <CardContent className="overflow-auto p-0">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {sales.map((sale) => {
                const total = sale.items.reduce((sum, item) => sum + (item.total ?? 0), 0);
                ...
                return (
                  <tr key={sale.id} className="border-t">
                    ...
                    <td className="p-3">{formatPLNFromGrosze(total)}</td>
                    <td className="p-3">
                      {paid
                        ? sale.payments
